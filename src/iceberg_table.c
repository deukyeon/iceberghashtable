...
{
   return nonzero_fprint(MurmurHash64A(key, KEY_SIZE, seed[0]));
}

static inline uint64_t
lv2_hash(KeyType key, uint8_t i)
{
   return nonzero_fprint(MurmurHash64A(key, KEY_SIZE, seed[i + 1]));
}
...
static inline bool
iceberg_key_compare(KeyType a, KeyType b)
{
...
static inline void
atomic_write_128(KeyType key, ValueType val, uint64_t *slot)
{
...
iceberg_init(iceberg_table *table, uint64_t log_slots)
{
   ...
   if ((table->level1[0] = (iceberg_lv1_block *)pmem_map_file(
           level1_filename,
           FILE_SIZE,
   ...
   if ((table->level2[0] = (iceberg_lv2_block *)pmem_map_file(
           level2_filename,
           FILE_SIZE,
           PMEM_FILE_CREATE | PMEM_FILE_SPARSE,
           0666,
   ...
   if ((table->level3[0] = (iceberg_lv3_list *)pmem_map_file(
           level3_filename,
           FILE_SIZE,
           PMEM_FILE_CREATE | PMEM_FILE_SPARSE,
           0666,
...
#   endif
   ...
   sprintf(level1_filename, "%s/level1", PMEM_PATH);
   if ((table->level1[0] = (iceberg_lv1_block *)pmem_map_file(
           level1_filename, 0, 0, 0666, &mapped_len, &is_pmem))
       == NULL)
   {
      perror("pmem_map_file");
      exit(1);
   ...
   sprintf(level2_filename, "%s/level2", PMEM_PATH);
   if ((table->level2[0] = (iceberg_lv2_block *)pmem_map_file(
           level2_filename, 0, 0, 0666, &mapped_len, &is_pmem))
       == NULL)
   {
      perror("pmem_map_file");
      exit(1);
   ...
   sprintf(level3_filename, "%s/level3", PMEM_PATH);
   if ((table->level3[0] = (iceberg_lv3_list *)pmem_map_file(
           level3_filename, 0, 0, 0666, &mapped_len, &is_pmem))
       == NULL)
   {
      perror("pmem_map_file");
      exit(1);
   ...
   char   level3_nodes_filename[FILENAME_LEN];
   sprintf(level3_nodes_filename, "%s/level3_data", PMEM_PATH);
   table->level3_nodes = (iceberg_lv3_node *)pmem_map_file(
      level3_nodes_filename, 0, 0, 0666, &mapped_len, &is_pmem);
   if (table->level3_nodes == NULL) {
      perror("pmem_map_file");
      exit(1);
   }
   ...
   for (uint64_t i = 1; i <= resize_cnt; ++i) {
      ...
      table->metadata.lv1_md[i] = (iceberg_lv1_block_md *)mmap(
         NULL, lv1_md_size, PROT_READ | PROT_WRITE, mmap_flags, 0, 0);
      ...
      table->metadata.lv2_md[i] = (iceberg_lv2_block_md *)mmap(
         NULL, lv2_md_size, PROT_READ | PROT_WRITE, mmap_flags, 0, 0);
      ...
      table->metadata.lv3_locks[i] = (uint8_t *)mmap(NULL,
                                                     sizeof(uint8_t) * nblocks,
                                                     PROT_READ | PROT_WRITE,
                                                     mmap_flags,
                                                     0,
                                                     0);
      ...
      for (uint64_t i = 0; i < total_blocks; i++) {
         ...
         for (uint64_t slot = 0; slot < 1 << SLOT_BITS; slot++) {
            ...
            if (key != 0) {
               ...
               get_index_offset(
                  table->metadata.log_init_size, index, &bindex, &boffset);
               assert(bindex == p);
               assert(boffset == i);
         ...
         for (uint64_t slot = 0; slot < C_LV2 + MAX_LG_LG_N / D_CHOICES; slot++)
         {
            ...
            if (key != 0) {
               ...
               if (bindex != p || boffset != i) {
                  split_hash(
                     lv2_hash(key, 1), &fprint, &index, &table->metadata);
                  get_index_offset(
                     table->metadata.log_init_size, index, &bindex, &boffset);
               }
               assert(bindex == p);
...
#   ifdef ENABLE_RESIZE
   ...
   for (uint64_t i = 0; i <= resize_cnt; ++i) {
      ...
      table->metadata.lv1_resize_marker[i] = (uint8_t *)mmap(
         NULL, resize_marker_size, PROT_READ | PROT_WRITE, mmap_flags, 0, 0);
      if (!table->metadata.lv1_resize_marker[i]) {
         perror("level1 resize ctr malloc failed");
         exit(1);
      }
      table->metadata.lv2_resize_marker[i] = (uint8_t *)mmap(
         NULL, resize_marker_size, PROT_READ | PROT_WRITE, mmap_flags, 0, 0);
      if (!table->metadata.lv2_resize_marker[i]) {
         perror("level2 resize ctr malloc failed");
         exit(1);
      }
      table->metadata.lv3_resize_marker[i] = (uint8_t *)mmap(
         NULL, resize_marker_size, PROT_READ | PROT_WRITE, mmap_flags, 0, 0);
                   ...
                   uint8_t        thread_id)
{
   ...
   if (iceberg_lv2_insert_internal(
          table, key, value, fprint1, index1, thread_id))
      return true;
                      ...
                      bool           overwrite_value_if_exist)
{
   ...
   bool ret = iceberg_insert_internal(
      table, key, value, fprint, bindex, boffset, thread_id);
   if (!ret)
...
iceberg_insert(iceberg_table *table,
               ...
               uint8_t        thread_id)
{
   return iceberg_put_or_insert(table, key, value, thread_id, false);
}
...
iceberg_put(iceberg_table *table,
            ...
            uint8_t        thread_id)
{
   return iceberg_put_or_insert(table, key, value, thread_id, true);
}
                   ...
                   uint8_t        thread_id)
{
   ...
   bool ret = iceberg_lv3_remove_internal(table, key, lv3_index, force_remove, thread_id);
                ...
                && lv3_index >= (table->metadata.nblocks >> 1)))
   {
      ...
      if (__atomic_load_n(&table->metadata.lv3_resize_marker[mindex][moffset],
          ...
          == 0)
      { // not fixed yet
         return iceberg_lv3_remove_internal(table, key, old_index, force_remove, thread_id);
      } else {
                   ...
                   && index >= (table->metadata.nblocks >> 1)))
      {
         ...
         { // not fixed yet
            ...
            for (uint8_t i = 0; i < popct; ++i) {
               ...
               if (iceberg_key_compare(blocks[boffset].slots[slot].key, *key)
                   == 0) {
                  if (blocks[old_boffset].slots[slot].val.refcount == 1 || force_remove) {
                     metadata->lv2_md[bindex][old_boffset].block_md[slot] = 0;
                     *key = blocks[old_boffset].slots[slot].key;
                     blocks[old_boffset].slots[slot].key          = NULL;
                     blocks[old_boffset].slots[slot].val.refcount = 0;
                     pc_add(&metadata->lv2_balls, -1, thread_id);
                     ret = true;
...
#   if PMEM
                  pmem_persist(&blocks[old_boffset].slots[slot],
                               sizeof(kv_pair));
#   endif
      ...
      for (uint8_t i = 0; i < popct; ++i) {
         ...
         if (iceberg_key_compare(blocks[boffset].slots[slot].key, *key) == 0) {
            if (blocks[boffset].slots[slot].val.refcount == 1 || force_remove) {
   ...
   return iceberg_lv3_remove(table, key, lv3_index, force_remove, thread_id);
}

static inline bool
iceberg_remove_and_get_key_with_force(iceberg_table *table,
                           KeyType       *key,
                           bool force_remove,
                           uint8_t        thread_id)
{
                ...
                && index >= (table->metadata.nblocks >> 1)))
   {
      ...
      { // not fixed yet
         ...
         for (uint8_t i = 0; i < popct; ++i) {
            ...
            {
               if (blocks[old_boffset].slots[slot].val.refcount == 1 || force_remove) {
                  metadata->lv1_md[bindex][old_boffset].block_md[slot] = 0;
                  *key = blocks[old_boffset].slots[slot].key;
                  blocks[old_boffset].slots[slot].key          = NULL;
                  blocks[old_boffset].slots[slot].val.refcount = 0;
                  pc_add(&metadata->lv1_balls, -1, thread_id);
...
#   endif
   ...
   ret = iceberg_lv2_remove(table, key, index, force_remove, thread_id);
...
{
   return iceberg_remove_and_get_key_with_force(table, &key, false, thread_id);
}
...
{
   return iceberg_remove_and_get_key_with_force(table, &key, true, thread_id);
}

static inline bool
iceberg_lv3_get_key_value_internal(iceberg_table *table,
                                   KeyType       *key,
                                   ValueType    **value,
                                   uint64_t       lv3_index)
{
...
iceberg_lv3_get_value_internal(iceberg_table *table,
                               ...
                               uint64_t       lv3_index)
{
   return iceberg_lv3_get_key_value_internal(table, &key, value, lv3_index);
}

static inline bool
iceberg_lv3_get_key_value(iceberg_table *table,
                          KeyType       *key,
                          ValueType    **value,
                          uint64_t       lv3_index)
{
                ...
                && lv3_index >= (table->metadata.nblocks >> 1)))
   {
      ...
      if (__atomic_load_n(&table->metadata.lv3_resize_marker[mindex][moffset],
          ...
          == 0)
      { // not fixed yet
         return iceberg_lv3_get_key_value_internal(
            table, key, value, old_index);
      } else {
         // wait for the old block to be fixed
         uint64_t dest_chunk_idx = lv3_index / 8;
         get_index_offset(table->metadata.log_init_size - 3,
...
iceberg_lv3_get_value(iceberg_table *table,
                      ...
                      uint64_t       lv3_index)
{
   return iceberg_lv3_get_key_value(table, &key, value, lv3_index);
}

static inline bool
iceberg_lv2_get_key_value(iceberg_table *table,
                          KeyType       *key,
                          ValueType    **value,
                          uint64_t       lv3_index)
{
                   ...
                   && index >= (table->metadata.nblocks >> 1)))
      {
         ...
         { // not fixed yet
            ...
            while (md_mask != 0) {
               ...
               if (iceberg_key_compare(blocks[old_boffset].slots[slot].key,
                                       *key)
                   == 0) {
                  *key   = blocks[old_boffset].slots[slot].key;
...
iceberg_lv2_get_value(iceberg_table *table,
                      ...
                      uint64_t       lv3_index)
{
   return iceberg_lv2_get_key_value(table, &key, value, lv3_index);
}
                      ...
                      uint8_t        thread_id)
{
                ...
                && index >= (table->metadata.nblocks >> 1)))
   {
      ...
      { // not fixed yet
         ...
         while (md_mask != 0) {
            ...
            if (iceberg_key_compare(blocks[old_boffset].slots[slot].key, *key)
                == 0) {
               *key   = blocks[old_boffset].slots[slot].key;
               *value = &blocks[old_boffset].slots[slot].val;
               return true;
   ...
   bool ret = iceberg_lv2_get_key_value(table, key, value, index);
...
iceberg_get_value(iceberg_table *table,
                  ...
                  uint8_t        thread_id)
{
   return iceberg_get_key_value(table, &key, value, thread_id);
}
   ...
   for (uint64_t j = 0; j < C_LV2 + MAX_LG_LG_N / D_CHOICES; ++j) {
      ...
      for (int i = 0; i < D_CHOICES; ++i) {
         ...
         if ((l2index & mask) == bnum && l2index != bnum) {
            if (!iceberg_lv2_insert_internal(
                   table, key, value, l2fprint, l2index, thread_id))
            {
